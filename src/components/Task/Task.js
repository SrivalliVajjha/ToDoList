...
import { UilEditAlt, UilCheck, UilTimes } from '@iconscout/react-unicons';
import "./Task.css";

export default function Task({ task, handleCompleteTask, handleEditTitle }) {
    ...
    <li className="task" key={task.id}>
      {isEdit ?
        <div className='editDiv'>
          ...
          <button title="Save" className="checkIcon" onClick={handleOnSubmitEditText}><UilCheck className="icon" size="18" color="#000" /></button>
          <button title="Cancel" className="timesIcon" onClick={handleEdit}><UilTimes className="icon" size="18" color="#000" /></button>
        </div> :
        <div class="initView">
